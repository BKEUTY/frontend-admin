                                        ...
                                        {optionTypes.map((opt, index) => (
                                            <div key={index} className="pc-option-group">
                                                <div className="pc-option-header">
                                                    ...
                                                    {opt.isCustomName && <CInput value={opt.optionName} onChange={(e) => handleOptionNameChange(index, e.target.value)} placeholder={t('admin_placeholder_option_name_custom')} style={{ flex: 1 }} />}
                                                    {index > 0 && <CButton type="text" danger icon={<DeleteOutlined />} onClick={() => handleRemoveOptionType(index)} />}
                                                </div>
                                                <Select mode="tags" className="pc-w-100" placeholder={t('admin_placeholder_option_values_short')} value={opt.optionValues} onChange={(val) => handleOptionValuesChange(index, val)} options={(!opt.isCustomName && availableOptions[opt.optionName]) ? availableOptions[opt.optionName].map(v => ({ value: v, label: v })) : []} />
                                            </div>
                                        ))}
